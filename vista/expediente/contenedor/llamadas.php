...
require_once($_SESSION['ROOT_PATH']."/controlador/c_llamada.php");
						...
						<form id="form_llamadas" action="#">
							<fieldset>					
								<div class="row">
									<section class="col col-5">				
										...
										<div id="alert_css">{ll?_tab1} <button type="button" class="confirm_button" onclick="v_llamada_enviarDatos()">{llb_tab3}</button><button type="reset" class="cancele_button" onclick="v_llamada_cancelarAd()" >{llb_tab4}</button></div>
							...
							if(isset($_POST['recargar'])){
                                                             ...
                                                             if (isset($_POST['guardar'])){
                                                              ...
                                                              //   print_r($_POST);
                                                                    ...
                                                                    $llamada->guardarTelefono($persona, $cod_area, $telefono, $usuario);
							...
							$llamadas = $llamadas.'
								<table style="font-size: 11px; width: 10%" cellpadding="0" cellspacing="0" border="0" class="display dataTable" id="telefonos-Tabla">
									<thead>
										<tr>
										<th width="20px">{tll_tab1}</th>
										<th width="20px">{tll_tab2}</th>
										<th width="20px">{tll_tab3}</th>
										</tr>
									</thead>
									<tbody>
				...
				<tfoot id="pietelefonos-Tabla">
					<tr>
						...
						<th></th>
                                                ...
                                                <form class="sky-form">
						<button name="Enviar" type="button" onclick="mostrarAdvertencia()" disabled  class="button" id="llamada_guardar">{llb_tab1}</button>
                                                <button type="button" class="button" onclick="agregarTelefono(personaActual.value);" id="btnAgregarTelefono">{llb_tab5}</button>
						<button type="button" class="button" onclick="v_llamada_activarEdicion();" id="llamada_edit_reset">{llb_tab2}</button>
                                               
</form>
							</fieldset>					
						</form>
				<script src="../controlador/c_llamada.js"></script>		
